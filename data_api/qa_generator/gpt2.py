...
class GPT2QAGenerator(LLMQAGenerator):
    ...
    def __init__(self):
        super().__init__('gpt2')  # Call the constructor of LLMQAGenerator with model_name = 'gpt2''
        # self.model_name = additional_value

    def load_llm(self, model_name):
        pipe = pipeline('text-generation', model=model_name)
        set_seed(seed = 21)
        print(f"""New text generation pipeline of model: {model_name} is setup.
    ...
    def run_model_inference(self, prompt: str) -> str:
        ...
        """
        output = self.pipeline(prompt, max_length=1000, truncation=True)[0]['generated_text']
        print(f'\n' + output + '\n')
        # return output
