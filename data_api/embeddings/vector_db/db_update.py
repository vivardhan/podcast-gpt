...
from data_api.chapterizer.transcript_chapterizer import convert_timestamp_string_to_milliseconds
...
def create_chapter_title_to_timestamps_dict(chapters_metadata: List[Tuple[str, str]]) -> Dict[str, Tuple[int, int]]:
	second_timestamps = [
	...
	for index in range(num_chapters):
		...
		next_index = index + 1
		end_timestamp = second_timestamps[next_index] if next_index < num_chapters else None
		title_to_timestamps[chapters_metadata[index][1]] = (start_timestamp, end_timestamp)
	...
	def generate_and_store_embeddings(cls, podcast_names: List[str]) -> None:
		...
		for podcast_name in podcast_names:
			...
			for chapterized_file in tqdm(chapterized_files):
				...
				metadata_file = Paths.get_metadata_file_path(podcast_name, episode_title)
				metadata = json.loads(GCSClient.download_textfile_as_string(metadata_file))
				chapter_title_to_timestamps = create_chapter_title_to_timestamps_dict(metadata[MetadataKeys.CHAPTERS_KEY])
				...
				for chapter_title, chapter_text in chapters.items():
					...
					start_timestamp, end_timestamp = chapter_title_to_timestamps[chapter_title]
					...
					data.append({
							...
							VectorDBConstants.EPISODE_URL_FIELD: metadata[MetadataKeys.URL_KEY],
							VectorDBConstants.PODCAST_GUEST_FIELD: metadata[MetadataKeys.GUEST_KEY],
