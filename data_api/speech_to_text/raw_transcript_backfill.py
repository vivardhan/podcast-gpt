...
def download_all_raw_transcript_texts() -> Dict[str, str]:
    ...
    for podcast_name in ["hubermanlab", "PeterAttiaMD"]:
        ...
        raw_text_folder = os.path.join(
            podcast_name, Paths.TEXT_DATA_FOLDER, Paths.RAW_TRANSCRIPT_FOLDER
        )
...
def match_aai_transcript_to_gcs_transcript(
    aai_transcript_text: str, gcs_transcripts: Dict[str, str]
) -> str:
    """
        ...
        try:
            ...
            if "transcripts" in r:
                for t in r["transcripts"]:
                    ...
                    file_name = match_aai_transcript_to_gcs_transcript(
                        aai_transcript["text"], gcs_transcripts
                    )
                    if file_name:
                        print(
                            "Matched transcript id: {} to file: {}".format(
                                t["id"], file_name
                            )
                        )
                        ...
                        print(
                            "Failed to match transcript id: {} to any gcs file".format(
                                t["id"]
                            )
                        )
